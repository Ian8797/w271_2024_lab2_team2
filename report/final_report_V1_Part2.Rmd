...
line_comp <- ggplot(present_data, aes(x = Month, y = CO2, color = Model)) +
  ...
  theme_minimal()+
  scale_color_brewer(palette = "Set1",labels = c("Actual Values", "Predicted Linear", "Predicted Quad", "Predicted Seasonal"))
...
forecast_data <- bind_rows(
  as_tibble(forecasts_ma, .name_repair = "minimal") %>% mutate(Model = "ARIMA MA"),
  as_tibble(forecasts_ar, .name_repair = "minimal") %>% mutate(Model = "ARIMA AR"),
  as_tibble(forecasts_full, .name_repair = "minimal") %>% mutate(Model = "ARIMA Full")
)
...
arima_comp <- ggplot(data = combined_data) +
  ...
  scale_color_manual(
    name = "Model",
    values = c("Actual" = "red", "ARIMA AR" = "darkgreen", "ARIMA Full" = "blue", "ARIMA MA" = "pink"),
    labels = c("Actual Values", "ARIMA AR", "ARIMA Full", "ARIMA MA")
  ) +
  scale_linetype_manual(
    name = "Model",
    values = c("Actual" = "solid", "ARIMA AR" = "dotted", "ARIMA Full" = "dashed", "ARIMA MA" = "longdash"),
    labels = c("Actual Values", "ARIMA AR", "ARIMA Full", "ARIMA MA")
  ) +
  guides(colour = guide_legend(order = 1), linetype = guide_legend(order = 1)) +
  labs(
    title = "Actual vs. Predicted ARIMA Models CO2 Levels",
    x = "Month",
    y = "CO2 Levels"
  ) +
  theme_minimal()
...
In the comparison of linear models, the forecast from the Predicted Seasonal model aligns closely with the actual CO2 values. The Predicted Linear model consistently underestimates the CO2 levels. The Predicted Quad model shows an improved fit in terms of the overall upward trend compared to the simple linear model, but it doesn't fully capture the periodic seasonal variations. Turning to the ARIMA models, forecasts from all variants—AR, MA, and Full—align closely with the actual CO2 values. 

```{r best seasonal arima comp, echo=FALSE}
#best seasonal arima = SARIMA(0,1,1)(0,1,1)_{12}
subset_df <- co2_monthly_present[co2_monthly_present$average > 420, ]
subset_df <- subset_df[1, ]
print(subset_df)
```

In 1997, our forecasts estimated when atmospheric CO2 levels might first surpass the 420 ppm threshold. The best-performing model, a Seasonal ARIMA, suggested that this milestone would be reached in different years under various scenarios: May 2017 was projected as a high extreme case, May 2023 as the model's central prediction, and May 2036 as a low extreme case. Actual measurements indicate that this level was first crossed in April 2022, as shown by the data extracted from the Mauna Loa Observatory. This outcome demonstrates that the Seasonal ARIMA model provided a reasonable estimation, with the actual occurrence falling between the predicted central and high extreme case scenarios. 
...
model_names <- c("predicted_linear", "predicted_quad", "predicted_seasonal", "ARIMA AR", "ARIMA MA", "ARIMA Full")
...
Looking at the performance metrics of all three linear and arima models, we see that the predicted_linear model shows high errors across all metrics. In contrast, both the predicted_quad and predicted_seasonal models demonstrate lower error values, suggesting a more accurate representation of the data trends. The ARIMA models—ARIMA AR, ARIMA MA, and ARIMA Full—present moderate errors, with ARIMA MA and ARIMA Full yielding similar performance metrics. Among the models assessed, the predicted_seasonal model stands out with the lowest RMSE value at 0.5776, highlighting its accuracy in tracking the actual data points. Moreover, all models have ACF1 values below 1, pointing to a lack of strong autocorrelation in the residuals and suggesting that each model is capturing the seasonality in the data to some extent.
