...
```
According to the above, the residuals of SARIMA model appear to be close to white noise. There is no significant lags, but with some seasonal pattern that suggests we should run a statistical test on the residuals from the models to see if they are randomly distributed i.e. are white noise, which is what we want for a good model fit, or if they appear to have some serial correlation over time and violate the assumptions for a stationary time series fit. And the Box-Ljung test result p-value of 0.53 > 0.05, it looks like the residuals of the SARIMA model are stationary and appeared to be white noise. As a result, we decided to use the SARIMA model to forecast using our test data.
